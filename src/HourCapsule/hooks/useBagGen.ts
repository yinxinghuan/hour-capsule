...
export interface BagSpec {
  ...
  /** Drawn from the tier-weighted domain pool (80/15/5). The bag picks
   *  up a different visual treatment per tier:
   *    common   — standard white α
   *    uncommon — silver α + faint silver shimmer along plastic ridges
   *    rare     — gold α + gold filigree along the heat-sealed strips
   */
  rarity?: Rarity;
}
...
function buildPrompt(spec: BagSpec): string {
  ...
  const mfgInk = spec.isShiftChange
    ...
    : "white inkjet ink";
  // Rarity-driven visual treatments — applied AFTER the core layout so
  // they override the default α color and add a small ornamental layer.
  const rarity = spec.rarity ?? 'common';
  const alphaInk =
    rarity === 'rare'
      ? "soft warm GOLD ink (NOT white — gold metallic foil)"
      : rarity === 'uncommon'
      ? "cool SILVER ink (NOT white — silver metallic foil)"
      : "MUTED FADED WHITE ink WITH SLIGHT TRANSPARENCY (NOT pure bright white, NOT fully opaque, softly translucent)";
  const rarityFlourish =
    rarity === 'rare'
      ? "RARITY TREATMENT: delicate fine GOLD filigree ornamental linework decorates the heat-sealed top and bottom seal strips — intricate gold motifs printed in metallic gold ink along the seal bands, like a luxury packaging detail. The plastic film also has faint warm gold iridescent highlights along a few ridges. "
      : rarity === 'uncommon'
      ? "RARITY TREATMENT: the plastic film carries a subtle SILVER iridescent shimmer along some of the ridges and folds, like rare metallic flecks baked into the polymer — cool, restrained, not glittery. "
      : "";
  return [
